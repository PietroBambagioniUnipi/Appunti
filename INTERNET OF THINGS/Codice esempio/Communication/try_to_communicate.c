...
static linkaddr_t dest_addr = {{0x00f4, 0x00ce, 0x0036, 0x0043, 0x0001, 0x000c3, 0x005d, 0x00cd}};
...
PROCESS_THREAD(comm_proc, ev, data){
    ...
    while(1) {
        ...
        if(etimer_expired(&periodic_timer)&&  !linkaddr_cmp(&dest_addr, &linkaddr_node_addr)){
        ...
        if(etimer_expired(&broadcast_periodic_timer) && !linkaddr_cmp(&dest_addr, &linkaddr_node_addr)){
